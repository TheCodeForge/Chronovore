...
class CustomRenderer(HTMLRenderer):
    ...
    def render_keyword(self, token):
        ...
        return f'<span type="button" class="keyword text-nowrap" data-bs-toggle="popover" data-bs-title="{kwd}" data-bs-content="{rule}" data-bs-custom-class="keyword-popover">{token.target}</span>'
...
class NumberedRenderer(CustomRenderer):
    ...
    def render_h_block(self, token):
        ...
        output = f'<h{tier} id="{snake}" class="clipboard-copy" data-clipboard-text="https://{request.host}{request.path}#{snake}">{header_raw}</h{tier}><div class="h-block">{self.render_inner(token)}</div>'
